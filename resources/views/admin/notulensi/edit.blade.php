...
@php
  // Parse existing attendees to array for checking
  $existingAttendees = array_map('trim', explode(',', $notulensi->attendees ?? ''));
@endphp
      ...
      <form action="{{ route('admin.notulensi.update', $notulensi) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <input type="text" name="title" value="{{ old('title', $notulensi->title) }}" required maxlength="255"
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input type="text" name="location" value="{{ old('location', $notulensi->location) }}" maxlength="255"
                class="w-full px-4 py-2 border border-himka-secondary/20 rounded-lg focus:outline-none focus:border-himka-accent">
              ...
              @if($pengurusAktif->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                  @foreach($pengurusAktif as $p)
                    @php
                      $attendeeValue = $p->user->name . ' (' . $p->jabatan . ')';
                      $isChecked = in_array($attendeeValue, old('attendees_list', [])) || 
                                   collect($existingAttendees)->contains(fn($a) => str_contains($a, $p->user->name));
                    @endphp
                    <label class="flex items-center gap-3 p-2 rounded-lg hover:bg-white cursor-pointer transition-colors">
                      <input type="checkbox" name="attendees_list[]" value="{{ $attendeeValue }}" 
          ...
          <div>
            ...
            @if($notulensi->attachment)
              <div class="mb-2 p-3 bg-himka-cream/50 rounded-lg flex items-center gap-3">
                <span class="material-icons text-himka-secondary">attach_file</span>
                <div class="flex-1">
                  <p class="text-sm text-himka-secondary">File saat ini</p>
                  <a href="{{ Storage::url($notulensi->attachment) }}" target="_blank" class="text-xs text-himka-accent hover:underline">Download / Lihat File</a>
                </div>
              </div>
            @endif
            <input type="file" name="attachment"
              class="w-full px-4 py-2 border border-himka-secondary/20 rounded-lg focus:outline-none focus:border-himka-accent">
            <p class="text-xs text-himka-secondary/50 mt-1">Maksimal 5MB. Format yang didukung: PDF, DOC, DOCX, XLS, XLSX. Upload file baru akan mengganti file lama.</p>
          </div>
        </div>
